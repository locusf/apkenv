...
EGLBoolean my_eglInitialize(EGLDisplay dpy, EGLint *major, EGLint *minor)
{
    WRAPPERS_DEBUG_PRINTF("eglInitialize\n");
    return eglInitialize(dpy,major,minor);
}

EGLBoolean my_eglTerminate(EGLDisplay dpy)
{
    WRAPPERS_DEBUG_PRINTF("eglTerminate\n");
    return eglTerminate(dpy);
}

const char * my_eglQueryString(EGLDisplay dpy, EGLint name)
{
...
EGLBoolean my_eglDestroySurface(EGLDisplay dpy, EGLSurface surface)
{
...
EGLBoolean my_eglBindAPI(EGLenum api)
{
    WRAPPERS_DEBUG_PRINTF("eglBindAPI\n");
    return eglBindAPI(api);
}

EGLenum my_eglQueryAPI(void)
{
    WRAPPERS_DEBUG_PRINTF("eglQueryAPI\n");
    return eglQueryAPI();
}

EGLBoolean my_eglWaitClient(void)
{
    WRAPPERS_DEBUG_PRINTF("eglWaitClient\n");
    return eglWaitClient();
}

EGLBoolean my_eglReleaseThread(void)
{
...
EGLBoolean my_eglBindTexImage(EGLDisplay dpy, EGLSurface surface, EGLint buffer)
{
    WRAPPERS_DEBUG_PRINTF("eglBindTexImage\n");
    return eglBindTexImage(dpy,surface,buffer);
}

EGLBoolean my_eglReleaseTexImage(EGLDisplay dpy, EGLSurface surface, EGLint buffer)
{
...
EGLBoolean my_eglSwapInterval(EGLDisplay dpy, EGLint interval)
{
...
EGLBoolean my_eglDestroyContext(EGLDisplay dpy, EGLContext ctx)
{
...
EGLContext my_eglGetCurrentContext(void)
{
    WRAPPERS_DEBUG_PRINTF("eglGetCurrentContext\n");
    return eglGetCurrentContext();
}

EGLSurface my_eglGetCurrentSurface(EGLint readdraw)
{
    WRAPPERS_DEBUG_PRINTF("eglGetCurrentSurface\n");
    return eglGetCurrentSurface(readdraw);
}

EGLDisplay my_eglGetCurrentDisplay(void)
{
...
EGLBoolean my_eglWaitGL(void)
{
    WRAPPERS_DEBUG_PRINTF("eglWaitGL\n");
    return eglWaitGL();
}

EGLBoolean my_eglWaitNative(EGLint engine)
{
